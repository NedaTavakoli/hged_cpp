...
void read_pos_substring_file(const std::string& pos_substring_file_name,
                        std::vector<int> &positions, 
                        std::vector<std::vector<std::string> > &substrings){
    std::ifstream infile(pos_substring_file_name);
    std::string line;
    ...
    while (std::getline(infile, line)){
        ...
        std::vector<std::string> row_of_substrings;
        iss >> pos;
        positions.push_back(pos);

       if (line.size() > 0){
            while (iss >> substring){
                row_of_substrings.push_back(substring); 
            }
        }
        substrings.push_back(row_of_substrings); 
    }
}
...
void create_alignment_graph(igraph_t* variation_graph, int pos, string s, int alpha, int delta, igraph_t* alignment_graph){ // commented by Neda
...
int main(int argc, char** argv){
    ...
    for(int i = 0; i < N; i++){
        for(int j = 0 ; j < substrings[i].size(); j++){       
            ...
            igraph_t alignment_graph;
            // create_alignment_graph(&variation_graph, positions[i], substrings[i], alpha, delta, &alignment_graph); //commented by Neda
            create_alignment_graph(&variation_graph, positions[i], substrings[i][j], alpha, delta, &alignment_graph); // Added by Neda
            ...
            //igraph_cattribute_GAS_set(&alignment_graph, "substring", substrings[i].c_str()); //commented by Neda
            igraph_cattribute_GAS_set(&alignment_graph, "substring", substrings[i][j].c_str()); // Added by Neda
